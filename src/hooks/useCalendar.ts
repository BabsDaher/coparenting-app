...
export function useCalendar() {
  ...
  async function setOvernight(dateKey: string, overnight: Overnight | undefined) {
    const ref = doc(db, 'calendar', DOC_ID)
    await setDoc(ref, { [dateKey]: { ...getDay(dateKey), overnight: overnight ?? null } }, { merge: true })
  }
  ...
  async function addEvent(dateKey: string, event: Omit<CalEvent, 'id'>) {
    ...
    const ref = doc(db, 'calendar', DOC_ID)
    await setDoc(ref, { [dateKey]: { ...day, events: [...day.events, newEvent] } }, { merge: true })
  }

  async function removeEvent(dateKey: string, eventId: string) {
    const day = getDay(dateKey)
    const ref = doc(db, 'calendar', DOC_ID)
    await setDoc(ref, { [dateKey]: { ...day, events: day.events.filter(e => e.id !== eventId) } }, { merge: true })
  }

  async function setNote(dateKey: string, note: string) {
    const ref = doc(db, 'calendar', DOC_ID)
    await setDoc(ref, { [dateKey]: { ...getDay(dateKey), note } }, { merge: true })
  }
